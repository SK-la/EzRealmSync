...
- `official-write/`：**只在测试输出里**（造官方参考库、跑官方 DLL 打开验收）。产品不引用这个工程，发布包不带它，也不在运行时启动它。
...
2. 在产品进程里加载任何版本的 `osu.Game.dll`（测试工程除外；「转官方」是与源库同版的**收窄迁移**，由 `DynamicOfficialConverter` 在进程内完成，不借 Worker）。
