...
| **dll / `osu.Game.dll`** | **同步不用它打开用户库**。仅测试验证、数据页完整 Ez 模型、修复升级时使用 |
...
- 同步只拷官方基线（谱面 / 收藏夹 / 皮肤 / `File` 与对应 `files/`），**不改**两边 schema，也不写 Ez 专用列。  
- 同步**不会**用 `osu.Game.dll` 打开用户库，避免错版本污染。要升级请用修复页或官方客户端。

---

## 数据页打不开旧版 Ez？

同步本身不依赖旧版 DLL。若**数据页**要用完整 Ez 模型打开更旧 schema：
