...
- **对比两份库（A / B）**并复制谱面集、难度、收藏夹、皮肤、成绩（不改 schema）
- 浏览、删除、导出数据；备份与还原
- 把 Ez 库按官方 schema 收窄成官方可用的 `client.realm`（原文件先备份）
...
1. 打开 **导入** 页  
2. 在「osu! 数据目录」里填或选到你的 osu! / Ez2Lazer **数据根目录**（下面应有 `client.realm` 或若干 `.realm`，以及共享的 `files/`），点 **应用路径**  
3. 点 **刷新列表**，确认列表里出现 Realm 文件  
4. 需要改库前，建议先用本页做一次 **备份**

设置、备份、导出、schema 快照与日志都在 **exe 同目录**：`settings.json`、`backups/`、`exports/`、`snapshots/`、`log/`。界面可在状态栏切换中文 / English。
...
| **数据** | 浏览单库内容；可删谱面集 / 成绩 / 收藏夹；导出文件、导入合集 | 删除是软删：谱面集 / 成绩置 `DeletePending`，收藏夹整行移除；下次有客户端打开该库时才生效 |
| **同步** | 选 A / B 两份库，按集合运算对比后，把谱面集 / 难度 / 收藏夹 / 皮肤 / 成绩复制到操作目标 | **不升级** schema；跨版本可读。类型选「全部」时**不含成绩**，要比成绩请选「成绩」 |
| **修复** | 把 Ez 库按官方 schema 收窄成官方 `client.realm`；修非法字符 / 缺文件 / 僵尸文件 | 转官方会先备份原文件，官方认不出的行只留在备份里（见下文） |
| **导出** | 批量导出谱包 / 谱面 / 收藏夹谱面 / 成绩 `.osr` / `collection.db` / `scores.db` | 谱面与回放依赖导入页目录下的 `files/`；名单类 db 不依赖 files |
...
1. 导入页选好数据目录并刷新  
2. 同步页选 A、B → 计算集合 → 在「仅 A」等标签里勾选 → 选「操作」（添加 / 删除）与「操作目标」→ 执行  
...
1. 关游戏 → 备份
2. 修复页选中该 Realm → **转回官方版**（原文件会先自动备份）
3. 若提示缺该版本的官方 schema，按提示指定一份同版本的官方 `client.realm`（用官方客户端在独立数据目录跑一次即得）

收窄后 Ez 专有的表、列与官方认不出的行只留在备份里；需要留着就先导出一份或先做 A/B 同步。

**导出收藏夹名单给稳定客户端**

1. 导出页类型选 **合集 (collection.db)** → 加载列表 → 勾选 → 导出  
2. 或在数据页对收藏夹右键「导出合集 (collection.db)…」；反过来也能用「导入合集 (collection.db)…」把名单并回 Realm  
...
| **schema 快照** | 工具读过的每个库版本会落一份结构快照（`snapshots/`）；「转回官方版」用它当收窄目标，缺对应版本时会要求指定一份官方库 |
| **files/** | 谱面、回放、皮肤等实体文件目录；修复 / 导出谱面与 `.osr` 需要它 |
...
- 数据页删除对官方库同样可用，且只落在官方 schema 本来就有的行列上；库版本号、Ez 扩展列都不会被改动。  
- 同步**不会**用 `osu.Game.dll` 打开用户库：读写全走 DynamicRealm，产品进程不加载它。要迁移版本请用 Ez2Lazer 客户端打开一次。  
- 「转回官方版」只搬官方客户端认得出的行：Ez 规则集（如 `diva` / `bms`）、外部托管谱面集、隐藏难度、Ez 判定语义或 Ez 专用 mod 的成绩都会被丢掉，并在结果里计数说明。
...
| `osu.Game.EzRealmSync.OfficialSchema` / `OfficialWrite` | 官方 schema 镜像与 Worker；只服务测试（造官方参考库、官方 DLL 打开验收），产品不引用、发布包不带 |
