...
namespace osu.Game.EzRealmSync.Tests
{
    ...
    public class DynamicDumpParityTest
    {
        ...
        /// 用真实数据而不是手搭对象：链接、嵌入、文件用法这些结构手搭最容易漏，漏了 parity 也跟着漏。
        /// </summary>
        private static void seedFromOfficialSample(string targetPath, int targetSchema)
        {
            ...
            if (sample == null)
                Assert.Ignore("未放置官方样本，跳过基于真实数据的 parity。");
            ...
            if (bundle.Scores.Count == 0)
            {
                ...
                TestContext.Out.WriteLine("官方样本没有可同步的成绩，本次 parity 不覆盖 Score。");
            }
